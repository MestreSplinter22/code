...
export const createHomeController = (productService: ProductService) => {
  ...
  app.get('/', async (c) => {
    ...
    // 1. Verifica a existência do cookie de autenticação
    const authToken = getCookie(c, 'auth_token');
    const isAuthenticated = !!authToken; // Converte para booleano

    return c.html(
      // 2. Passamos isAuthenticated para o Layout (ajuste a interface do Layout se necessário)
      <Layout title="Adsly - Agência de Contingência" isAuthenticated={isAuthenticated}>
        
        {/* 3. Passamos isAuthenticated para a Navbar */}
        ...
        <footer className="border-t border-zinc-800 py-8 mt-8 text-center text-zinc-600 text-sm">
